...
# from torchvision.models.resnet import lenet
from LeNetPruned import MyLeNet5 as lenet
...
parser.add_argument(
    '--model_dir',
    default="model",
    help='Trained model file path. Download pretrained model from the following url and put it in model_dir specified path: https://download.pytorch.org/models/lenet-5c106cde.pth'
)
parser.add_argument(
    '--config_file',
    default=None,
...
def load_data(train=True,
  ...
  if train:
    ...
    dataset = torchvision.datasets.MNIST(root='./data', train=True, transform=data_transform, download=True)
    if subset_len:
    ...
    data_transform = transforms.Compose([transforms.ToTensor()])
    dataset = torchvision.datasets.MNIST(root='./data', train=False, transform=data_transform, download=True)
...
def evaluate(model, val_loader, loss_fn):
  ...
  for iteraction, (images, labels) in tqdm(
    ...
    # print(outputs[0])
    predicted, actual = classes[torch.argmax(outputs[0])], classes[labels]
    # print(predicted)
    # print(actual)
