...
def get_image_from_url(url=''):
    ...
    # img_transforms = opencv_transform.Compose(
    #         # [   opencv_transform.Resize(int(math.ceil(28 / 0.875)),interpolation=cv2.INTER_LINEAR),
    #         #     opencv_transform.CenterCrop(28),
    #             [opencv_transform.ToTensor(),
    #             # opencv_transform.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    #         ])
    # # download the image from web if uri is valid.
    ...
    print(f"HEREEE: {torch.unsqueeze(tensor_image, 0).size()}")
    
    return torch.unsqueeze(tensor_image, 0)
...
def main(): 
    ...
    wego_mod = wego_torch.compile(mod, wego_torch.CompileOptions(
        inputs_meta = [
        wego_torch.InputMeta(torch.float, [1, 1, 28, 28])
        ]
    ))
    wego_mod.eval()
